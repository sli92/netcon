...
Wird nun ein UDP-Antwortpaket vom Server an die Anwendung ausgesendet, enthält es ein JSON-Objekt, das laut folgender Seite formatiert ist, wenn beispielsweise zwei Module mit jeweils zwei Devices im Netzwerk gefunden wurden. Konnte der netcon Daemon keine Module im Netzwerk finden, sieht das JSON-Objekt folgendermaßen aus:
\begin{quote}
\begin{verbatim}
{"modulliste":null}
\end{verbatim}
...
Am Beispiel der bereits programmierten Website wird im Abschnitt Website die Verwendung der in JavaScript integrierten JSON-Bibliothek erläutert und weiters der Aufbau der gesamten Website geschildert. Dazu wird zunächst ein PHP-fähiger Webserver benötigt (siehe Installation). 
\newpage
...
\section[Installation] {Installation\footnote{\textit{Hinweis:} Der Diplomarbeit liegt eine CD bei, mit dem Java Daemon (netcond.jar) und der Beispielwebsite (netcon web).}}
