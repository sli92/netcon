...
private static String group = "255.255.255.255";
	...
	public static void sendBroadcast(byte msg[], int dstPort, int srcPort) throws IOException {
			
			SocketAddress inet = new InetSocketAddress(InetAddress.getLocalHost(), srcPort);
		
			MulticastSocket socket = new MulticastSocket(inet);
			
			DatagramPacket msgPacket = new DatagramPacket(msg, msg.length, InetAddress.getByName(group), dstPort);
